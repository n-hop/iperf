...
void iperf_set_test_state(struct iperf_test *ipt, signed char state) {
  if (ipt->debug_level >= DEBUG_LEVEL_INFO) {
    iperf_printf(ipt, "State change: State set to %d-%s (from %d-%s)\n", state,
                 state_to_text(state), ipt->state, state_to_text(ipt->state));
  }
  ipt->state = state;
}
...
void iperf_set_test_json_callback(struct iperf_test *ipt,
                                  void (*callback)(struct iperf_test *,
                                                   char *)) {
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv) {
  ...
  while ((flag = getopt_long(
              argc, argv,
              "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:mhX:",
              longopts, NULL)) != -1) {
    switch (flag) {
    case 'p':
      portno = atoi(optarg);
    ...
    case OPT_CNTL_KA:
      ...
      // Seems that at least in Windows WSL2, TCP keepalive retries full inteval
      // must be smaller than the idle interval. Otherwise, the keepalive
      // message is sent only once.
      if (test->settings->cntl_ka_keepidle &&
          test->settings->cntl_ka_keepidle <=
              (test->settings->cntl_ka_count *
               test->settings->cntl_ka_interval)) {
        i_errno = IECNTLKA;
        return -1;
      }
      break;
...
#if defined(HAVE_CLOCK_NANOSLEEP) || defined(HAVE_NANOSLEEP)
  // If estimated time to next send is large enough, sleep instead of just CPU
  // looping until green light is set
  if (missing_rate < 0) {
    ...
    if (time_to_green_light >= 0) {
      ...
      // Sleep until average baud rate reaches the target value or intrupt /
      // error
      do {
        // nansleep() time should be less than 1 sec
        nanosleep_time.tv_nsec = (time_to_green_light >= SEC_TO_NS)
                                     ? SEC_TO_NS - 1
                                     : time_to_green_light;
...
int iperf_send_mt(struct iperf_stream *sp) {
  ...
  throttle_check_per_message =
      test->settings->rate != 0 && test->settings->burst == 0;
#endif /* HAVE_CLOCK_NANOSLEEP, HAVE_NANOSLEEP */
...
#if defined(HAVE_CLOCK_NANOSLEEP) || defined(HAVE_NANOSLEEP)
  /* Should check if green light can be set, as pacing timer is not supported in
   * this case */
  if (throttle_check && (!throttle_check_per_message || message_sent == 0)) {
#else  /* !HAVE_CLOCK_NANOSLEEP && !HAVE_NANOSLEEP */
  if (!throttle_check_per_message ||
      message_sent == 0) { /* Throttle check if was not checked for each send */
#endif /* HAVE_CLOCK_NANOSLEEP, HAVE_NANOSLEEP */
    iperf_time_now(&now);
    iperf_check_throttle(sp, &now);
  }
...
int iperf_recv_mt(struct iperf_stream *sp) {
  ...
  /* Collect statistics only if receive did not timeout (e.g. `Nread()` may
   * timeout). This is also important for `--rcv-timeout` to work properly.
   */
  if (r > 0) {
...
int iperf_create_send_timers(struct iperf_test *test) {
  ...
  SLIST_FOREACH(sp, &test->streams, streams) { sp->green_light = 1; }
  return 0;
}
...
int test_is_authorized(struct iperf_test *test) {
  ...
  if (test->settings->authtoken) {
    ...
    int rc = decode_auth_setting(test->debug, test->settings->authtoken,
                                 ...
                                 &password, &ts, test->use_pkcs1_padding);
    if (rc) {
      return -1;
    }
...
static int get_parameters(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "server_affinity",
                                             cJSON_Number)) != NULL)
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "blockcount", cJSON_Number)) !=
        NULL)
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "nodelay", cJSON_True)) != NULL)
      test->no_delay = 1;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "parallel", cJSON_Number)) !=
        NULL)
      test->num_streams = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "reverse", cJSON_True)) != NULL)
      iperf_set_test_reverse(test, 1);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "bidirectional", cJSON_True)) !=
        NULL)
      iperf_set_test_bidirectional(test, 1);
#if defined(HAVE_IPPROTO_MPTCP)
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "mptcp", cJSON_True)) != NULL)
      test->mptcp = 1;
#endif
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "window", cJSON_Number)) !=
        NULL)
      test->settings->socket_bufsize = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "len", cJSON_Number)) != NULL)
      test->settings->blksize = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "bandwidth", cJSON_Number)) !=
        NULL)
      test->settings->rate = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "fqrate", cJSON_Number)) !=
        NULL)
      test->settings->fqrate = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "pacing_timer",
                                             cJSON_Number)) != NULL)
    ...
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "flowlabel", cJSON_Number)) !=
        NULL)
      test->settings->flowlabel = j_p->valueint;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "title", cJSON_String)) != NULL)
      test->title = strdup(j_p->valuestring);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "extra_data", cJSON_String)) !=
        NULL)
      test->extra_data = strdup(j_p->valuestring);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "congestion", cJSON_String)) !=
        NULL)
      test->congestion = strdup(j_p->valuestring);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "congestion_used",
                                             cJSON_String)) != NULL)
      test->congestion_used = strdup(j_p->valuestring);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "get_server_output",
                                             cJSON_Number)) != NULL)
      iperf_set_test_get_server_output(test, 1);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "udp_counters_64bit",
                                             cJSON_Number)) != NULL)
      iperf_set_test_udp_counters_64bit(test, 1);
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "repeating_payload",
                                             cJSON_Number)) != NULL)
      test->repeating_payload = 1;
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "zerocopy", cJSON_Number)) !=
        NULL)
      test->zerocopy = j_p->valueint;
#if defined(HAVE_DONT_FRAGMENT)
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "dont_fragment",
                                             cJSON_Number)) != NULL)
      test->settings->dont_fragment = j_p->valueint;
#endif /* HAVE_DONT_FRAGMENT */
#if defined(HAVE_SSL)
    if ((j_p = iperf_cJSON_GetObjectItemType(j, "authtoken", cJSON_String)) !=
        NULL)
...
static int get_results(struct iperf_test *test) {
  ...
  if (j == NULL) {
    ...
    j_cpu_util_total =
        iperf_cJSON_GetObjectItemType(j, "cpu_util_total", cJSON_Number);
    j_cpu_util_user =
        iperf_cJSON_GetObjectItemType(j, "cpu_util_user", cJSON_Number);
    j_cpu_util_system =
        iperf_cJSON_GetObjectItemType(j, "cpu_util_system", cJSON_Number);
    j_sender_has_retransmits = iperf_cJSON_GetObjectItemType(
        j, "sender_has_retransmits", cJSON_Number);
      ...
      if (j_streams == NULL) {
        ...
        for (i = 0; i < n; ++i) {
          ...
          if (j_stream == NULL) {
            ...
            j_bytes =
                iperf_cJSON_GetObjectItemType(j_stream, "bytes", cJSON_Number);
            j_retransmits = iperf_cJSON_GetObjectItemType(
                j_stream, "retransmits", cJSON_Number);
            j_jitter =
                iperf_cJSON_GetObjectItemType(j_stream, "jitter", cJSON_Number);
            j_errors =
                iperf_cJSON_GetObjectItemType(j_stream, "errors", cJSON_Number);
            j_omitted_errors = iperf_cJSON_GetObjectItemType(
                j_stream, "omitted_errors", cJSON_Number);
            j_packets = iperf_cJSON_GetObjectItemType(j_stream, "packets",
                                                      cJSON_Number);
            j_omitted_packets = iperf_cJSON_GetObjectItemType(
                j_stream, "omitted_packets", cJSON_Number);
            j_start_time = iperf_cJSON_GetObjectItemType(j_stream, "start_time",
                                                         cJSON_Number);
            j_end_time = iperf_cJSON_GetObjectItemType(j_stream, "end_time",
                                                       cJSON_Number);
        ...
        if (test->role == 'c' && iperf_get_test_get_server_output(test)) {
          ...
          if (j_server_output != NULL) {
             ...
             * later. */
            j_server_output = iperf_cJSON_GetObjectItemType(
                j, "server_output_text", cJSON_String);
            if (j_server_output != NULL) {
    ...
    j_remote_congestion_used =
        iperf_cJSON_GetObjectItemType(j, "congestion_used", cJSON_String);
...
static cJSON *JSON_read(int fd, int max_size) {
  ...
  if (rc == sizeof(nsize)) {
    ...
    if (hsize > 0 && (max_size == 0 || hsize <= max_size)) {
          ...
          if (rc >= 0) {
            ...
            if (rc == hsize) {
              ...
              snprintf(msg_buf, sizeof(msg_buf),
                       "JSON size of data read does not correspond to offered "
                       "length - expected %d bytes but received %d; errno=%d",
                       hsize, rc, errno);
              warning(msg_buf);
            }
          } else {
            snprintf(msg_buf, sizeof(msg_buf),
                     "JSON data read failed; errno=%d", errno);
            warning(msg_buf);
          }
          free(str);
        }
      ...
      snprintf(msg_buf, sizeof(msg_buf),
               "JSON data length overflow - %d bytes JSON size is not allowed",
               hsize);
      warning(msg_buf);
    }
  } else {
    ...
    snprintf(msg_buf, sizeof(msg_buf),
             "Failed to read JSON data size - read returned %d; errno=%d", rc,
             errno);
    warning(msg_buf);
  }
  return json;
...
void add_to_interval_list(struct iperf_stream_result *rp,
  ...
  /* Only the last interval result is needed, so removing last old entry to
   * reduce memory consupmtion */
...
struct iperf_test *iperf_new_test() {
  ...
  memset(test->bitrate_limit_intervals_traffic_bytes, 0,
         sizeof(iperf_size_t) * MAX_INTERVAL);
...
static void iperf_print_intermediate(struct iperf_test *test) {
   ...
   * results around unless we're the server and the client requested the server
   * output.
   *
   * This avoids unneeded memory build up for long sessions.
   */
  discard_json =
      test->json_stream == 1 && !(test->role == 's' && test->get_server_output);
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                  ...
                  iperf_json_printf(
                      ...
                      bandwidth * 8, retransmits, irp->omitted,
...
static void print_interval_results(struct iperf_test *test,
  ...
  if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
    if ((test->sender_has_retransmits == 1 || test->protocol->id == Psctp) &&
        sp->sender) {
...
void iperf_got_sigend(struct iperf_test *test, int sig) {
  ...
  if (test->ctrl_sck >= 0) {
    iperf_set_test_state(test, (test->role == 'c') ? CLIENT_TERMINATE
                                                   : SERVER_TERMINATE);
...
#endif
  if (exit_normal) {
    iperf_signormalexit(test, "interrupt - %s by signal %s(%d)",
                        iperf_strerror(i_errno), strsignal(sig), sig);
  } else {
    iperf_errexit(test, "interrupt - %s by signal %s(%d)",
                  iperf_strerror(i_errno), strsignal(sig), sig);
  }
}
...
int iperf_json_finish(struct iperf_test *test) {
  if (test->json_top) {
    ...
    if (test->json_stream) {
      cJSON *error =
          iperf_cJSON_GetObjectItemType(test->json_top, "error", cJSON_String);
      if (error) {
...
int iperf_set_control_keepalive(struct iperf_test *test) {
  ...
  if (test->settings->cntl_ka) {
    ...
    opt = 1;
    if (setsockopt(test->ctrl_sck, SOL_SOCKET, SO_KEEPALIVE, (char *)&opt,
                   sizeof(opt))) {
      i_errno = IESETCNTLKA;
      return -1;
    }
    ...
    if ((kaidle = test->settings->cntl_ka_keepidle) == 0) {
      len = sizeof(kaidle);
      if (getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPIDLE, (char *)&kaidle,
                     &len)) {
    ...
    if ((kainterval = test->settings->cntl_ka_interval) == 0) {
      ...
      if (getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPINTVL,
                     (char *)&kainterval, &len)) {
        i_errno = IESETCNTLKAINTERVAL;
        return -1;
      }
    ...
    if ((kacount = test->settings->cntl_ka_count) == 0) {
      len = sizeof(kacount);
      if (getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPCNT, (char *)&kacount,
                     &len)) {
    ...
    // Seems that at least in Windows WSL2, TCP keepalive retries full inteval
    // must be smaller than the idle interval. Otherwise, the keepalive message
    // is sent only once.
    if (test->settings->cntl_ka_keepidle) {
      if (test->settings->cntl_ka_keepidle <= (kainterval * kacount)) {
        iperf_err(test,
                  "Keepalive Idle time (%d) should be greater than "
                  "Retries-interval (%d) times Retries-count (%d)",
    ...
    if ((opt = test->settings->cntl_ka_keepidle)) {
      if (setsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPIDLE, (char *)&opt,
                     sizeof(opt))) {
    ...
    if ((opt = test->settings->cntl_ka_interval)) {
      if (setsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPINTVL, (char *)&opt,
                     sizeof(opt))) {
        i_errno = IESETCNTLKAINTERVAL;
        return -1;
      }
    ...
    if ((opt = test->settings->cntl_ka_count)) {
      if (setsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_KEEPCNT, (char *)&opt,
                     sizeof(opt))) {
        i_errno = IESETCNTLKACOUNT;
        return -1;
      }
    ...
    if (test->verbose) {
      printf("Control connection TCP Keepalive "
             "TCP_KEEPIDLE/TCP_KEEPINTVL/TCP_KEEPCNT are set to %d/%d/%d\n",
             kaidle, kainterval, kacount);
    }
  }
