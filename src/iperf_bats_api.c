...
void *__iperf_server_worker_run(void *s) {
...
void bats_connection_callback(bats_connection_handle_t conn,
                              bats_conn_event_t event,
...
void bats_listener_callback(bats_connection_handle_t conn,
                            bats_listen_event_t event, void *user_data) {
  switch (event) {
...
void handle_accepted_stream(struct iperf_test *test,
  ...
  if (test->state != CREATE_STREAMS) {
    ...
    if (test->bats_ctrl == NULL) {
      ...
      if (bats_connection_send_data(conn, (char *)&rbuf, sizeof(rbuf)) !=
          bats_error_none) {
        if (test->debug)
...
int iperf_bats_server_listen(struct iperf_test *test) {
retry:
  // Set configuration parameters
  if (test->bats_io == NULL) {
    test->bats_io = bats_context_create();
    if (!test->bats_io) {
      iperf_printf(test,
                   "[bats_server_example] Failed to create BATS context\n");
      return -1;
    }
    bats_context_set_log_level(test->bats_io, bats_log_level_info);
  }

  if (test->bats_config == NULL) {
    test->bats_config = bats_config_create();
    if (!test->bats_config) {
      iperf_printf(
          test, "[bats_server_example] Failed to create BATS configuration\n");
      bats_context_destroy(test->bats_io);
      return -1;
    }
    bats_config_set_mode(test->bats_config, 10); // bats_transport_btp
    bats_config_set_local_port(test->bats_config, test->server_port);
    bats_config_set_timeout(test->bats_config, 2);
    bats_config_set_cc(test->bats_config, bats_cc_bbr);
    // bats_config_cert_file(test->bats_config, cert_file);
    // bats_config_key_file(test->bats_config, key_file);
  }

  if (test->bats_listener == NULL) {
    test->bats_listener =
        bats_protocol_create(test->bats_io, test->bats_config);
    if (!test->bats_listener) {
      iperf_printf(test, "Failed to create BATS protocol\n");
  ...
  bats_error_t err = bats_protocol_start_listen(
      test->bats_listener, test->bind_address, test->server_port,
      bats_listener_callback, NULL);
  if (err != bats_error_none) {
    iperf_printf(test, "[bats_server_example] Failed to start listening: %d\n",
                 err);
...
int iperf_run_bats_server(struct iperf_test *test) {
  ...
  if (iperf_bats_server_listen(test) < 0) {
    cleanup_server(test);
    return -2;
  }
    ...
    if (result > 0) {
      ...
      if (test->state == CREATE_STREAMS) {
        ...
        if (rec_streams_accepted == streams_to_rec &&
          ...
          SLIST_FOREACH(sp, &test->streams, streams) {
            if (pthread_create(&(sp->thr), &attr, &__iperf_server_worker_run,
                               sp) != 0) {
