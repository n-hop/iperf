...
struct iperf_settings {
...
#endif              // HAVE_SSL
  int skip_rx_copy; /* Whether to ignore received messages data, using MSG_TRUNC
                       option */
  ...
  int cntl_ka_keepidle; /* Control TCP connection Keepalive idle time
                           (TCP_KEEPIDLE) */
  int cntl_ka_interval; /* Control TCP connection Keepalive interval between
                           retries (TCP_KEEPINTV) */
  int cntl_ka_count;    /* Control TCP connection Keepalive number of retries
                           (TCP_KEEPCNT) */
};
...
struct iperf_stream {
  ...
  int remote_port;
  int socket;
  bats_connection_handle_t bats_conn;
  int id;
  int sender;
...
struct iperf_test {
  ...
  void (*json_callback)(struct iperf_test *,
                        char *); /* allow user apps to receive the
